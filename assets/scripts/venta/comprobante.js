function BuscarClientes(){
    ...
    input.autocomplete({
        ...
        select: function (e, ui) {
            ...
            {
            	alert('Este cliente no tiene Rfc, actualize su información para proceder con la factura.');
            }
...
function ValidarCampos()
{
	...
	if(v=='3')
	{
		...
		$("#trSubTotal").show();
		$("#trIva").show();
		$("#spIdentidad").text('RFC');
		$("#txtrfc").attr('placeholder', 'RFC');
	}else if(v=='0')
	{
		$("#txtCliente").attr('readonly', true);
		$("#spRfcRequerido").hide();
		$("#spIdentidad").text('CURP');
		$("#txtRfc").attr('placeholder', 'CURP');
	}
	else
	{
		$("#txtCliente").removeClass('required');
		$("#txtRfc").removeClass('required');
